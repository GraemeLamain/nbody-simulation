...
class BoundingBox:
    '''Square region of space centred at (cx, cy).'''
    ...
    def contains(self, position: np.ndarray) -> bool:
        # strict inequalities so bodies on shared edges fall into exactly one quadrant
...
class QuadTree:
    '''Single node in the Barnes-Hut quadtree.

    Leaf nodes hold bodies directly. Internal nodes have four children and no bodies.
    ...
    def insert(self, body: Body) -> bool:
        ...
        if not self.divided:
            ...
            if len(self.bodies) > self.MAX_CAPACITY:
                ...
                # if all bodies are at the exact same position they'll keep overflowing
                # the same child - nudge the newest one slightly to break the loop
    ...
    def compute_mass_distribution(self) -> None:
        '''Walk the tree bottom-up, computing total mass and CoM for each node.'''
        ...
        if not self.divided:
            ...
            # pre-compute flat arrays for the vectorized leaf evaluation
            n = len(self.bodies)
            if n > 0:
    ...
    def compute_force(self, body: Body, theta: float = 0.5, softening: float = 0.0) -> np.ndarray:
        '''Compute gravitational force on a single body by walking the tree.'''
        fx, fy = 0.0, 0.0
        stack  = [self]
        mu     = G * body.mass
    ...
    def flatten(self, id_to_idx: dict):
        '''Serialize the tree into flat numpy arrays for the Numba force kernel.

        Numba can't work with Python objects so we do a DFS and pack everything
        into parallel arrays indexed by integer node IDs. Root is always index 0.
        Child links are stored as integer indices (-1 = no child).
        '''
        MAX_NODES = max(4096, len(id_to_idx) * 4)

        node_cx         = np.empty(MAX_NODES, dtype=np.float64)
        node_cy         = np.empty(MAX_NODES, dtype=np.float64)
        node_mass       = np.empty(MAX_NODES, dtype=np.float64)
        node_half_w     = np.empty(MAX_NODES, dtype=np.float64)
        node_child      = np.full((MAX_NODES, 4), -1, dtype=np.int32)
        node_leaf_start = np.full(MAX_NODES, -1,  dtype=np.int32)
        node_leaf_count = np.zeros(MAX_NODES,      dtype=np.int32)
